(function(){
	...
	newsMod.loadInitNews(2).then(() => newsMod.addNews());
...
function NewsModule(modalWin){
	...
	let index;
	let loaded = false;
	NewsModule.prototype.loadInitNews =  async function (basis_count) {
	index = basis_count;
	return fetch("/js/json/6.json").then(response => response.json())
	.then(newsjson => {
		news = newsjson.map(function(news,i){
	...
	function scrollFunc() {
		if(loaded && document.getElementById("news-bar").getBoundingClientRect().bottom<window.innerHeight)
		{
