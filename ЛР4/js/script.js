...
function ModalWindows (time,objMsgBox)
{
	...
	document.getElementById("modal-cart-submit").addEventListener("click",function(){
		let keys = Object.keys(localStorage);
		let basket_info = [];
		for(let key of keys)
		{
			key = key.split('-');
			if (key[0] == "order")
				basket_info.push({"id" : key[1], "count" : localStorage.getItem(key[0]+"-"+key[1])});
		};
		const params = {
			method : "POST",
			body: JSON.stringify({"data" : basket_info}),
			headers: {
				'Content-Type': 'application/json'
			}
		};
		fetch("cart", params).then(response => response.json())
		.then(jsonData => {
			objMsgBox.showMsg(jsonData.title,jsonData.message,jsonData.type);
			if(jsonData.mtype == "ok")
			{
				document.getElementById("modal-cart-clean").click();
				document.getElementById("modal-cart-close").click();	
			}
			if(jsonData.redirect == "1")
				setTimeout(() => window.location.href = "login",2000);
		});
		fetch("advert", params).then(response => response.json())
		.then(jsonData => {
...
async function loadBasket () {
	...
	for(let key of keys)
	{
		...
		{
			ids.push(key[1]);
			count_good[key[1]] = localStorage.getItem(key[0]+"-"+key[1]);
		}
	};
	const params = {
		method : "POST",
		body: JSON.stringify({"good_load_type" : "idmutli","ids" : ids}),
		headers: {
			'Content-Type': 'application/json'
		}
	}
	let Goods = await fetch("goodEngine", params).then(response => response.json())
	.then(jsonData => {
		if(jsonData.mtype == "alert")
			objMsgBox.showMsg(jsonData.title,jsonData.message,jsonData.type);
		else if (jsonData.mtype == "update")
			return jsonData.data;
		else if (jsonData.mtype == "empty")
			return [];
		else
			alert("Something wrong");
	...
	for(let good of Goods)
		htmlblock += `<div data-good-id="${good.id}" class="cart-item">
								<img src="img/goods/${good.photoName}.${good.photoType}">
								<p class="cart-item-header">${good.title}</p>
								<p class="cart-item-price"><span class="cart-price-value">${good.price}</span> UAH.</p>
								<div class="item-count-module">
									<button class="item-count-add item-count-edit">+</button>
									<span class="item-count-value">${count_good[good.id]}</span>
...
async function Filter1 ()
{
	...
	let stackgoods;
	await fetch("goodEngine",params).then(response => response.json())
	.then(jsonData => {
		if(jsonData.mtype == "alert")
			objMsgBox.showMsg(jsonData.title,jsonData.message,jsonData.type);
		else if (jsonData.mtype == "update")
			stackgoods = jsonData.data;
		else
			alert("Something wrong");
	});
